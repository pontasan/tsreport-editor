...
일반적인 개발과 검증에서는 호스트의 `src/`에서도 npm 명령을 실행할 수 있습니다. Docker는 호스트와 분리되며, 의존성은 Node.js 이미지 빌드 시 lockfile에서 설치됩니다. 컨테이너 시작 시 `npm install`이나 `npm ci`를 실행하지 않고, Compose Watch는 호스트의 `node_modules`를 제외한 소스만 동기화합니다.
